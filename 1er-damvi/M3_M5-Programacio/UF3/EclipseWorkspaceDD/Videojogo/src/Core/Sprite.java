...
public class Sprite implements Serializable, Comparable<Sprite> {
	...
	//No tocar aquest boolean. Esta en package per alguna rao. El faig servir per comprovacions internes del s Phantoms
	...
	/**Determines the order in the field. 
	 * The higher the value the more in front will be. The highest value will be on the forefront and the lowest on the background.
	 */
	public Integer orderInLayer = 0;

	
	//Fer tots els canvis al constructor privat de Sprite. Si no m'oblidaré algun dia de fer el canvi a algun dels 40000 constructors i no trobaré mai el problema
	private Sprite(String name, int x1, int y1, int x2, int y2, double angle, Field f) {
		...
		if(f!=null)f.newSprites.add(this);
	}
	/**
	...
	public Sprite(String name, int x1, int y1, int x2, int y2, double angle, String path, Field f) {
        ...
        if(!fil.exists()) {
        	System.err.println("No hi ha cap imatge en "+path);
        }
	}
	...
	public boolean collidesWith(Sprite other) {
		if(!this.equals(other)&&!this.delete&&!this.internalDelete&&!other.delete&&!other.internalDelete&&other.solid&&this.solid) {
	...
	public String toString() {
		return "Sprite "+this.name+" , from ("+this.x1+", "+this.y1+") to ("+this.x2+", "+this.y2+") "+" url: "+this.path;
		
	}
	
	/**
	 * each turn the update function will be called for every non-deleted sprite in a field. 
	 */
	public void update() {
		//buida
	}
	...
	/**
	 * CompareTo orders the list according to the orderInLayer. This ensures that the higher sprites are drawn last
	 */
	@Override
